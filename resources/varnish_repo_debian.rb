...
action :configure do
  ...
  apt_repository "varnish-cache_#{new_resource.major_version}" do
    uri "https://packagecloud.io/varnishcache/varnish#{major_version_no_dot}lts/#{node['platform']}/"
    distribution node['lsb']['codename']
    components ['main']
    key "https://packagecloud.io/varnishcache/varnish#{major_version_no_dot}lts/gpgkey" if new_resource.fetch_gpg_key
    deb_src true
    action :add
  end
end
